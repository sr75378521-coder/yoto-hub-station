...
function renderErrorPage(error) {
	...
	if (errMsg.includes("Supabase environment variable") || errMsg.includes("Supabase client is not configured") || errMsg.includes("Missing Supabase environment variable")) return `<!doctype html>
<html lang="en">
  <head>
    <meta charset="utf-8" />
...
async function getServerEntry() {
	if (!serverEntryPromise) serverEntryPromise = import("./server-CujKjyX4.mjs").then((m) => m.default ?? m);
